...
def _init_attn_qk_pair_projection(
    weight: torch.Tensor,
    bias: Optional[torch.Tensor],
    *,
    head_dim: int,
    std: float,
    cutoff_factor: Optional[float],
    pair_same_init: bool,
    pair_opposite_bias: bool,
) -> None:
    """
    Apply the AttnSSM pair rule to a q/k projection tensor.

    The tensor is interpreted as a stack of heads, with `head_dim` rows per head.
    For AttnSSM the relevant heads are grouped as (0, 1), (2, 3), ... within the
    q or k projection output. The helper only touches the paired portion of the
    tensor, so an odd leftover head keeps its original initialization.
    """
    if weight.ndim != 2 or head_dim <= 0 or weight.shape[0] % head_dim != 0:
        return
    ...
    paired_rows = 2 * num_pairs * head_dim
    paired_weight = weight[:paired_rows].view(num_pairs * 2, head_dim, weight.shape[1])

    with torch.no_grad():
        if pair_same_init:
            # Make the two heads inside every pair start from the exact same weight
            # matrix. This preserves the original random draw for the first head and
            # simply mirrors it into the second head.
            paired_weight[1::2].copy_(paired_weight[0::2])

        if bias is None or not pair_opposite_bias:
            return

        paired_bias = bias[:paired_rows].view(num_pairs * 2, head_dim)

        # Biases are still initialized from a fresh random draw here, because the
        # requested "opposite bias" behavior needs a non-zero first half in order to
        # produce a meaningful negated second half. The default `init_normal()` path
        # still zeros biases when this flag is off.
        if cutoff_factor is not None:
            cutoff_value = cutoff_factor * std
            nn.init.trunc_normal_(paired_bias[0::2], mean=0.0, std=std, a=-cutoff_value, b=cutoff_value)
        else:
            nn.init.normal_(paired_bias[0::2], mean=0.0, std=std)
        paired_bias[1::2].copy_(-paired_bias[0::2])
...
def _default_mcs_lmax(config: ModelConfig) -> float:
    explicit_lmax = getattr(config, "mcs_L_max", None)
    if explicit_lmax is not None:
        return float(explicit_lmax)

    candidates: List[float] = []
    len_extra_orig_length = getattr(config, "len_extra_orig_length", 0)
    if isinstance(len_extra_orig_length, (int, float)) and len_extra_orig_length > 0:
        candidates.append(float(len_extra_orig_length))

    max_sequence_length = getattr(config, "max_sequence_length", None)
    if isinstance(max_sequence_length, (int, float)) and max_sequence_length > 0:
        candidates.append(float(max_sequence_length) * 16.0)

    return max(candidates) if candidates else 8192.0


def _build_mcs_positions(raw_gaps: torch.Tensor, log_lmin: float, log_lmax: float) -> torch.Tensor:
    span = max(log_lmax - log_lmin, 1e-6)
    gaps = F.softplus(raw_gaps) + 1e-6
    cumulative = torch.cumsum(gaps, dim=0)
    centers = (cumulative - 0.5 * gaps) / cumulative[-1]
    return log_lmin + span * centers


def _mcs_inv_freq(config: ModelConfig, dim: int) -> torch.Tensor:
    """
    Minimax Coverage Spectrum (MCS-RoPE).

    Solves for a log-spaced set of wavelengths whose smooth worst-case
    coverage over the target scale range is approximately uniform.
    """
    m = dim // 2
    if m <= 0:
    ...
    cache_key = (
        ...
        getattr(config, "len_extra_orig_length", None),
        getattr(config, "rope_theta", None),
        getattr(config, "mcs_L_min", 8.0),
        getattr(config, "mcs_L_max", None),
        getattr(config, "mcs_n_scales", None),
        getattr(config, "mcs_sigma", 0.2),
        getattr(config, "mcs_target_coverage", 1.0),
        getattr(config, "mcs_beta", 1e-3),
        getattr(config, "mcs_steps", 300),
        getattr(config, "mcs_lr", 0.05),
        getattr(config, "mcs_smooth_max_temperature", 0.05),
    )
    cached = getattr(config, "_mcs_inv_freq_cache", None)
    if cached is not None and cache_key in cached:
        return cached[cache_key].clone()

    l_min = float(getattr(config, "mcs_L_min", 8.0))
    l_max = _default_mcs_lmax(config)
    if l_min <= 0:
        raise ValueError("mcs_L_min must be positive")
    if l_max <= l_min:
        l_max = l_min * 2.0

    log_lmin = math.log(l_min)
    log_lmax = math.log(l_max)
    sigma = max(float(getattr(config, "mcs_sigma", 0.2)), 1e-3)
    target_coverage = float(getattr(config, "mcs_target_coverage", 1.0))
    beta = max(float(getattr(config, "mcs_beta", 1e-3)), 0.0)
    n_scales = getattr(config, "mcs_n_scales", None)
    n_scales = max(int(n_scales if n_scales is not None else max(4, 2 * m)), 2)
    n_steps = max(int(getattr(config, "mcs_steps", 300)), 1)
    lr = max(float(getattr(config, "mcs_lr", 0.05)), 1e-4)
    smooth_temp = max(float(getattr(config, "mcs_smooth_max_temperature", 0.05)), 1e-4)

    if m == 1:
        theta = torch.tensor([2 * math.pi / math.sqrt(l_min * l_max)], dtype=torch.float32)
    else:
        target_x = torch.linspace(log_lmin, log_lmax, n_scales, dtype=torch.float64)
        init_x = torch.linspace(log_lmin, log_lmax, m, dtype=torch.float64)
        init_gaps = torch.diff(init_x, prepend=init_x[:1])
        if init_gaps[0] <= 0:
            init_gaps[0] = max((log_lmax - log_lmin) / max(m, 1), 1e-3)
        raw_gaps = torch.nn.Parameter(torch.log(torch.expm1(init_gaps.clamp_min(1e-4))))
        optimizer = torch.optim.Adam([raw_gaps], lr=lr)

        best_loss = None
        best_x = init_x.clone()
        for _ in range(n_steps):
            optimizer.zero_grad()
            x = _build_mcs_positions(raw_gaps, log_lmin, log_lmax)
            kernel = torch.exp(-0.5 * ((x[:, None] - target_x[None, :]) / sigma) ** 2)
            coverage = kernel.sum(dim=0)
            deficit = F.relu(target_coverage - coverage)
            max_deficit = smooth_temp * torch.logsumexp(deficit / smooth_temp, dim=0)
            gaps = x[1:] - x[:-1]
            spacing_penalty = beta * torch.sum(torch.reciprocal(gaps.square() + 1e-8))
            loss = max_deficit + spacing_penalty
            loss.backward()
            optimizer.step()
        ...
        theta = (2 * math.pi) * torch.exp(-best_x)
        theta, _ = torch.sort(theta.to(torch.float32), descending=True)

    if cached is None:
        cached = {}
        setattr(config, "_mcs_inv_freq_cache", cached)
    cached[cache_key] = theta.detach().clone()
    return theta


def _yarn_attention_factor(config: ModelConfig) -> float:
    factor = float(getattr(config, "len_extra_yarn_scale", 1.0))
    if factor <= 1.0:
        return 1.0

    explicit = getattr(config, "attention_yarn_attention_factor", None)
    ...
    mscale = float(getattr(config, "attention_yarn_mscale", 1.0))
    mscale_all_dim = float(getattr(config, "attention_yarn_mscale_all_dim", 0.0))
    if mscale != 0.0 and mscale_all_dim != 0.0:
        return float(get_mscale(factor, mscale) / get_mscale(factor, mscale_all_dim))
    return float(get_mscale(factor))


def _attention_logit_scale(config: ModelConfig) -> float:
    temperature = max(float(getattr(config, "attention_temperature", 1.0)), 1e-6)
...
class RotaryEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if self.freq_distribution not in [
            "constant",
            "linear",
            "uniform",
            "gaussian",
    ...
    def extra_yarn2(self, inv_freq = None) -> torch.Tensor:
        assert self.config.len_extra_orig_length != 0
        ...
        len_extra_yarn_scale2 = self.config.len_extra_yarn_scale * (self.config.rope_theta ** (2 / self.dim))
        
        if inv_freq is None:
            pos_freqs = self.config.rope_theta ** (torch.arange(0, self.dim, 2, device=_non_meta_init_device(self.config), dtype=torch.float) / self.dim)
            inv_freq_extra = 1.0 / pos_freqs
            inv_freq_inter = 1.0 / (len_extra_yarn_scale2 * pos_freqs)
        else:
            inv_freq_extra = inv_freq
            inv_freq_inter = inv_freq / len_extra_yarn_scale2
    ...
    def get_inv_freq(self, dim: int, device: torch.device) -> torch.Tensor:
        ...
        elif self.freq_distribution in {"mcs", "mcs_rope", "minimax_coverage"}:
            inv_freq = _mcs_inv_freq(self.config, dim).to(device)
        else:
            inv_freq = 1.0 / (
        ...
        if self.config.len_extra and self.config.len_extra_before_clamp:
            ...
            elif self.config.len_extra_type == "YARN2":
                inv_freq = self.extra_yarn2(inv_freq)
    ...
    def get_rotary_embedding(
        ...
        with torch.autocast(device.type, enabled=False):
            ...
            elif getattr(self.config, "rope_extend_position", None) is not None:
                seq = (
                    torch.arange(seq_len, device=device, dtype=torch.float)
                    * float(self.config.rope_extend_position)
                )
            else:
    ...
    def apply_to_qk(
        ...
        q = self.forward(q, all_len, layer_idx=layer_idx, use_rope_cache=use_rope_cache)
        k = self.forward(k, all_len, layer_idx=layer_idx, use_rope_cache=use_rope_cache)
        return q, k
...
class MixedRotaryEmbedding(RotaryEmbedding):
    """
    MixedPE: interpolate between continuous RoPE phases and a straight-through
    discretized floor target on a per-frequency basis.
    """
    ...
    def __init__(self, config: ModelConfig, *args, **kwargs):
        ...
        alpha_init = float(getattr(self.config, "mixedpe_alpha_init", 0.0))
        alpha_init = min(max(alpha_init, 0.0), 1.0)
        alpha = torch.full(
            (self.dim // 2,),
            alpha_init,
        ...
        if getattr(self.config, "mixedpe_alpha_learnable", False):
            self.alpha = nn.Parameter(alpha)
        else:
            self.register_buffer("alpha", alpha, persistent=True)

    def _mix_alpha(self, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    ...
    def _mixed_floor(self, phases: torch.Tensor) -> torch.Tensor:
        alpha = self._mix_alpha(phases.device, phases.dtype)
        discrete = phases + (torch.floor(phases) - phases).detach()
        return phases + alpha * (discrete - phases)
    ...
    def get_rotary_embedding(
        ...
        if self.config.rope_no_pos:
            seq = torch.ones(seq_len, dtype=torch.float, device=device)
        elif getattr(self.config, "rope_extend_position", None) is not None:
            seq = (
                torch.arange(seq_len, dtype=torch.float, device=device)
                * float(self.config.rope_extend_position)
            )
        else:
            seq = torch.arange(seq_len, dtype=torch.float, device=device)

        inv_freq = self.inv_freq.to(device=device, dtype=seq.dtype)
        if self.prefix == "embed":
            phases = torch.einsum("t, d -> td", seq, inv_freq)
        elif self.prefix == "attn":
            phases = torch.einsum("t, hd -> htd", seq, inv_freq)
        else:
            raise ValueError(f"Unsupported prefix: {self.prefix}")

        mixed_phases = self._mixed_floor(phases)
        positions = torch.cat((mixed_phases, mixed_phases), dim=-1).unsqueeze(0)
        return positions.sin(), positions.cos()


class AttnSSMRotaryEmbedding(RotaryEmbedding):
    """
    Position-dependent Q/K modulation adapted from attn_ssm's retinal stereo path.

    Adjacent heads are grouped into pairs. Within each pair, Q/K are decomposed into
    average and disparity channels and modulated by position-dependent exponentials
    parameterized by per-feature tau values. The resulting Q/K features are rebuilt as
    two distinct heads per pair: one average head and one disparity head.

    This implementation intentionally matches the original full-sequence formulation and
    only supports q_len == k_len.
    """

    def __init__(self, config: ModelConfig, *args, **kwargs):
        kwargs["use_rope_cache"] = False
        super().__init__(config, *args, **kwargs)
        self.suffix = "attn_ssm"

        if self.prefix != "attn":
            raise ValueError("AttnSSMRotaryEmbedding only supports prefix='attn'")
    ...
    def _position_factors(self, pos: torch.Tensor, dtype: torch.dtype) -> Tuple[torch.Tensor, torch.Tensor]:
        device = pos.device
        tau = self.tau.to(device=device, dtype=torch.float32)
        exponent_q = -pos[None, :, None, None] / tau[None, None, :, :]
        exponent_k = pos[None, :, None, None] / tau[None, None, :, :]

        if self.exp_clip > 0:
            exponent_q = exponent_q.clamp(-self.exp_clip, self.exp_clip)
            exponent_k = exponent_k.clamp(-self.exp_clip, self.exp_clip)
    ...
    def apply_to_qk(
        ...
        if q.size(1) != self.config.n_heads:
            raise ValueError(
                f"AttnSSMRotaryEmbedding expects {self.config.n_heads} query heads, got {q.size(1)}"
            )
        if k.size(1) != self.config.n_heads:
            raise ValueError(
                "AttnSSMRotaryEmbedding requires standard multi-head attention with "
                f"{self.config.n_heads} key heads, got {k.size(1)}"
            )
        if q.size(-1) != self.head_dim or k.size(-1) != self.head_dim:
            raise ValueError(
                f"AttnSSMRotaryEmbedding expects head_dim={self.head_dim}, got q={q.size(-1)} k={k.size(-1)}"
            )
        ...
        pos = torch.arange(q_len, device=q_.device, dtype=torch.float32)
...
class OLMoBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        # These cty-only switches keep the public configuration surface small while
        # still letting q/k projections opt into special AttnSSM-aware behavior.
        # `attn_qk_include_bias` is an explicit override for q/k bias creation;
        # when it is omitted, q/k inherit the original `include_bias` setting.
        # `attn_qk_pair_opposite_bias` forces q/k bias creation because it cannot
        # work without a bias parameter to mirror.
        attn_qk_include_bias = getattr(config, "attn_qk_include_bias", None)
        if attn_qk_include_bias is None:
            attn_qk_include_bias = config.include_bias
        self.attn_qk_include_bias = bool(attn_qk_include_bias or getattr(config, "attn_qk_pair_opposite_bias", False))
        self.attn_qk_pair_same_init = bool(getattr(config, "attn_qk_pair_same_init", False))
        self.attn_qk_pair_opposite_bias = bool(getattr(config, "attn_qk_pair_opposite_bias", False))
        ...
        if self.config.rope:
            rope_variant = getattr(self.config, "rope_variant", "rope").lower()
            if self.config.fourier:
                if rope_variant not in {"rope", "default", "fourier"}:
                    raise ValueError("fourier cannot be combined with rope_variant=%s" % rope_variant)
                self.pos_emb = FourierEmbedding(config, self.__cache)
            elif rope_variant in {"mixed", "mixedpe", "mixed_pe"}:
                self.pos_emb = MixedRotaryEmbedding(config, self.__cache)
            elif rope_variant in {"attn_ssm", "attn-ssm", "attnssm"}:
        ...
        self.attention_logit_scale = _attention_logit_scale(config)
    ...
    def _scaled_dot_product_attention(
        ...
        if max_doc_len is not None and cu_doc_lens is not None:
            if q.size(-1) != k.size(-1) or q.size(-1) != v.size(-1):
                raise ValueError(
                    "document-masked attention requires matching q/k/v head dimensions, "
...
class OLMoSequentialBlock(OLMoBlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        self.attn_qk_q_bias: Optional[nn.Parameter]
        self.attn_qk_k_bias: Optional[nn.Parameter]
        if self.attn_qk_include_bias and not config.include_bias:
            # `att_proj` is fused, so changing its bias flag would also add a v
            # bias. Separate q/k-only parameters give the requested bias without
            # changing v or any unrelated projection when `include_bias=False`.
            self.attn_qk_q_bias = nn.Parameter(torch.zeros(self.fused_dims[0], device=config.init_device))
            self.attn_qk_k_bias = nn.Parameter(torch.zeros(self.fused_dims[1], device=config.init_device))
        else:
            self.register_parameter("attn_qk_q_bias", None)
            self.register_parameter("attn_qk_k_bias", None)
    ...
    def reset_attn_qk_pair_parameters(self, std: float, cutoff_factor: Optional[float]) -> None:
        q_proj_out_dim = self.fused_dims[0]
        k_proj_out_dim = self.fused_dims[1]
        q_weight = self.att_proj.weight[:q_proj_out_dim]
        k_weight = self.att_proj.weight[q_proj_out_dim : q_proj_out_dim + k_proj_out_dim]

        q_bias = None
        k_bias = None
        if self.att_proj.bias is not None:
            q_bias = self.att_proj.bias[:q_proj_out_dim]
            k_bias = self.att_proj.bias[q_proj_out_dim : q_proj_out_dim + k_proj_out_dim]
        elif self.attn_qk_q_bias is not None:
            q_bias = self.attn_qk_q_bias
            k_bias = self.attn_qk_k_bias
            torch.nn.init.zeros_(q_bias)
            torch.nn.init.zeros_(k_bias)

        if self.attn_qk_pair_same_init or self.attn_qk_pair_opposite_bias:
            # These slices correspond exactly to the q/k tensors that are later
            # reshaped into [B, H, T, D]. Therefore rows [2*i*D : (2*i+1)*D]
            # and [(2*i+1)*D : (2*i+2)*D] are the two halves of one AttnSSM pair.
            _init_attn_qk_pair_projection(
                q_weight,
                q_bias,
                head_dim=self.head_dim,
                std=std,
                cutoff_factor=cutoff_factor,
                pair_same_init=self.attn_qk_pair_same_init,
                pair_opposite_bias=self.attn_qk_pair_opposite_bias,
            )
            _init_attn_qk_pair_projection(
                k_weight,
                k_bias,
    ...
    def reset_parameters(self):
        ...
        self.reset_attn_qk_pair_parameters(std, cutoff_factor)
    ...
    def forward(
        ...
        if self.attn_qk_q_bias is not None:
            # The sequential block uses one fused qkv projection. When the model
            # globally disables linear biases but the cty q/k bias flag is on, we
            # add q/k-only bias here so v and all other linears remain unchanged.
            q = q + self.attn_qk_q_bias
            k = k + self.attn_qk_k_bias
...
class OLMoLlamaBlock(OLMoBlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        self.q_proj = nn.Linear(
            config.d_model,
            q_proj_out_dim,
            # q/k can opt into a bias independently from the rest of the model.
            bias=self.attn_qk_include_bias,
            device=config.init_device,
        )
    ...
    def reset_parameters(self):
        ...
        # The explicit q/k projection path can express the pair rule directly.
        # This is the cleanest place to apply the new cty switches because q and
        # k are already separate modules here, so no extra reshaping or slicing is
        # needed beyond the head-wise interpretation of their output rows.
        if self.attn_qk_pair_same_init or self.attn_qk_pair_opposite_bias:
            _init_attn_qk_pair_projection(
                self.q_proj.weight,
                self.q_proj.bias,

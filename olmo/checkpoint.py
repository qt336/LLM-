                        ...
                        if value.shape[-2] < new_value.shape[-2] or value.shape[-1] < new_value.shape[-1]:
                            if dist_model.module.config.len_extra_fourier_expand:
                                ...
                                def get_step_eye(_param):
                                    ...
                                    step = math.ceil(_param.shape[-2] / _param.shape[-1])
                                    for i in range(_param.shape[-1]):
                                        if i*step < _param.shape[-2]:
                                            _param[..., i*step, i] = 1.0
                                    
                                    return _param
                                
                                new_value += get_step_eye(new_value)
                            
                            new_value[..., :value.shape[-2], :value.shape[-1]] = value
                        else:
                            if value.shape[-2] > new_value.shape[-2] or value.shape[-1] > new_value.shape[-1]:
                                print(f"contract {key} from {value.shape} to {dist_model.module.state_dict()[key].shape}")
                            
                            new_value = value[..., :new_value.shape[-2], :new_value.shape[-1]]

...
class ModelConfig(BaseConfig):
    ...
    debug: bool = False
    """
    If ``True``, use special debug mode.
    """
    
    pos_emb: bool = True
    """
    If ``True``, use positional embeddings, including wte, alibi, rope, and freq_emb.
    """
    ...
    rope_init_distribution: str = "exponential"
    """
    The distribution setting for RoPE.
    """
    ...
    """
    Optional scale applied to position indices before rotary phases are computed.
    """
    
    rope_no_pos: bool = False
    """
    If ``True``, don't include positional index in the rotary embedding.
    """
    
    rope_no_rotary: bool = False
    """
    If ``True``, don't include the rotary frequency in different dimension.
    """
    
    rope_no_repetition: bool = False
    """
    If ``True``, don't include repetition frequency in different head.
    """
    ...
    """
    If ``True``, the inv_freqs in RoPE/iRoPE are learnable.
    """
    
    rope_include_neg_freq: bool = False
    """
    If ``True``, include negative frequencies in the rotary positional embeddings.
    """
    
    rope_init_upper_freq: float = 1.0
    """
    The upper frequency setting for RoPE.
    """
    
    rope_init_floor_freq: float = 0.0
    """
    The floor frequency setting for RoPE.
    """
    
    rope_floor_freq_ratio: float = 1.0
    """
    The floor freq will be ratio * 2 * pi / max_seq_len. If ratio=0.0, will be original RoPE.
    """
    
    rope_upper_freq_ratio: float = 1.0
    """
    The upper freq will be ratio * pi.
    """
    
    rope_clamp_floor_freq: bool = False
    """
    If ``True``, clamp the frequency in the rotary positional embeddings.
    """
    
    rope_clamp_floor_to_zero: bool = False
    """
    If ``True``, clamp the floor frequency to zero.
    """
    
    rope_clamp_upper_freq: bool = False
    """
    If ``True``, clamp the frequency in the rotary positional embeddings.
    """
    
    rope_clamp_upper_to_zero: bool = True
    """
    If ``True``, clamp the upper frequency to zero.
    """
    
    rope_zero_freq_ratio: float = -1.0
    """
    The number of zero freq will be ratio * dim. If ratio < 0, will be original RoPE.
    """

    mcs_L_min: float = 8.0
    """
    Lower wavelength bound used by MCS-RoPE spectrum construction.
    """

    mcs_L_max: Optional[float] = None
    """
    Optional upper wavelength bound used by MCS-RoPE spectrum construction.
    """

    mcs_n_scales: Optional[int] = None
    """
    Number of target log-scales used by the MCS-RoPE solver.
    """

    mcs_sigma: float = 0.2
    """
    Kernel width in log-wavelength space for MCS-RoPE coverage estimation.
    """

    mcs_target_coverage: float = 1.0
    """
    Target smooth coverage level for each log-scale in MCS-RoPE.
    """

    mcs_beta: float = 1e-3
    """
    Spacing regularization weight for MCS-RoPE.
    """

    mcs_steps: int = 300
    """
    Number of optimization steps used to solve the MCS-RoPE spectrum.
    """

    mcs_lr: float = 0.05
    """
    Learning rate used by the offline MCS-RoPE solver.
    """

    mcs_smooth_max_temperature: float = 0.05
    """
    Temperature for the smooth max objective in MCS-RoPE.
    """
    
    rope_clamp_to_linear: bool = False
    """
    If ``True``, clamp the frequency to linear.
    """
    
    rope_clamp_to_linear_mode: str = "ceil"
    """
    The mode to clamp the frequency to linear.
    """

    mixedpe_alpha_init: float = 0.0
    """
    Initial interpolation coefficient for MixedPE. ``0`` matches RoPE, ``1`` matches FloorPE.
    """

    mixedpe_alpha_learnable: bool = False
    """
    If ``True``, learn the MixedPE interpolation coefficients during training.
    """

    attn_ssm_tau_min: float = 8.0
    """
    Minimum tau used by the attn_ssm positional modulation.
    """

    attn_ssm_tau_ratio: float = 1.25
    """
    Geometric ratio used across successive attn_ssm feature pairs.
    """

    attn_ssm_pair_tau_ratio: float = 1.0
    """
    Optional geometric ratio applied across successive attn_ssm head pairs.
    """

    attn_ssm_tau_max: Optional[float] = None
    """
    Optional upper bound for attn_ssm tau values.
    """

    attn_ssm_disparity_eta: float = 1.0
    """
    Relative weight of the disparity branch in attn_ssm positional modulation.
    """

    attn_ssm_learn_eta: bool = False
    """
    If ``True``, learn the disparity weight in attn_ssm positional modulation.
    """

    attn_ssm_exp_clip: float = 20.0
    """
    Clip range for attn_ssm position exponents before exponentiation.
    """

    attn_ssm_center_positions: bool = True
    """
    If ``True``, center positions around the middle of the current sequence in
    ...
    fourier_dim: int = 0
    """
    The dimension to initialize the Fourier basis in RoPE.
    """
    
    fourier_init: str = "eye_xavier_norm"
    """
    The initialization strategy for the Fourier basis in RoPE.
    """
    
    fourier_init_norm_gain: float = 0.3
    """
    The normalization gain for the Fourier basis in RoPE.
    """

    fourier_separate_basis: bool = True
    """
    If ``True``, use seperate Fourier basis in RoPE.
    """
    
    fourier_separate_head: bool = True
    """
    If ``True``, use seperate Fourier basis in RoPE for each head.
    """
    
    fourier_learnable: bool = False
    """
    If ``True``, the Fourier basis in RoPE are learnable.
    """
    
    fourier_norm: bool = False
    """
    If ``True``, normalize the Fourier coefficients in Fourier.
    """
    ...
    freqwise_attention: bool = False
    """
    If ``True``, use frequency-wise attention.
    """
    ...
    """
    Softmax temperature applied to attention logits. Values below ``1`` sharpen attention.
    """

    attention_use_yarn_temperature: bool = False
    """
    If ``True``, multiply the attention logit scale by YaRN's recommended factor.
    """

    attention_yarn_attention_factor: Optional[float] = None
    """
    Optional explicit YaRN attention factor override. If ``None``, infer it from
    ``len_extra_yarn_scale`` using the YaRN default formula.
    """

    attention_yarn_mscale: float = 1.0
    """
    Optional YaRN ``mscale`` numerator used when inferring the attention factor.
    """

    attention_yarn_mscale_all_dim: float = 0.0
    """
    Optional YaRN ``mscale_all_dim`` denominator used when inferring the attention factor.
    """
    ...
    attention_layer_norm_v: bool = False
    """
    Apply layer norm to the values within the attention mechanism.
    This can help stabilize training.
    """
    
    attention_layer_norm_out: bool = False
    """
    Apply layer norm to the attn_out within the attention mechanism.
    This can help stabilize training.
    """
...
@dataclass
class PasskeyConfig(BaseConfig):
    min_tokens: int = 128
    max_tokens: int = 8192
    tokens_step: Optional[int] = None
    length_step: int = 128
    iterations: int = 20
    same_examples: bool = True
...
@dataclass
class GenerationEvaluatorConfig(BaseConfig):
    passkey: Optional[PasskeyConfig] = None
...
class EvaluatorConfig(BaseConfig):
    ...
    generation: GenerationEvaluatorConfig = field(default_factory=GenerationEvaluatorConfig)
    device_eval_batch_size: Optional[int] = None

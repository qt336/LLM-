...
class Trainer:
    ...
    def should_log_to_swanlab(self) -> bool:
        return self.cfg.swanlab is not None and self.swanlab_run is not None
    ...
    def load_trainer_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        # print(f"inv_freq_after_loading: {self.model.transformer.blocks[0].rotary_emb.inv_freq}")
        # exit() # dubug
    ...
    def model_forward(
        ...
        #     input_embeds_tmp = self.embed_tokens(input_ids)
        #     with torch.no_grad():
        #         logits_tmp = self.dist_model(
        #             input_embeddings=input_embeds_tmp,
        #             attention_mask=batch.get("attention_mask"),
        #             attention_bias=batch.get("attention_bias"),
        #             doc_lens=batch.get("doc_lens"),
        #             max_doc_lens=batch.get("max_doc_lens"),
        #         ).logits
        #         tokens_fur = torch.cat((input_ids[:, 0].unsqueeze(-1), torch.argmax(logits_tmp, dim=-1)), dim=-1)
        #         input_embeds_fur = self.embed_tokens(tokens_fur)
        #     
        #     input_embeds = (1 - self.cfg.intuitive_loss_alpha) * input_embeds_tmp + self.cfg.intuitive_loss_alpha * input_embeds_fur
    ...
    def eval_generation_step(self, batch: Dict[str, Any], evaluator: Evaluator) -> None:
        ...
        for eval_cfg in self.cfg.evaluators:
            if eval_cfg.label == evaluator.label:
                generation_cfg = eval_cfg.generation.passkey
                break
        assert generation_cfg is not None, f"Missing generation config for evaluator {evaluator.label}"

        if not hasattr(self, "_generation_eval_tokenizer"):
            self._generation_eval_tokenizer = Tokenizer.from_train_config(self.cfg)
        tokenizer = self._generation_eval_tokenizer

        self.dist_model.eval()
        with torch.no_grad():
            generations = []
            for idx in range(batch["input_ids"].shape[0]):
                prompt_len = int(batch["prompt_len"][idx].item())
                input_ids = batch["input_ids"][idx : idx + 1, :prompt_len]
                attention_mask = batch.get("attention_mask")
                if attention_mask is not None:
                    attention_mask = attention_mask[idx : idx + 1, :prompt_len]
                ...
                generations.append(tokenizer.decode(generated_tokens[0, prompt_len:], skip_special_tokens=True))

        assert isinstance(evaluator.eval_metric, PasskeyMetric)
        evaluator.eval_metric.update(batch, generations)
        barrier()
    ...
    def log_metrics_to_console(self, prefix: str, metrics: Dict[str, float]):
        ...
        # ! bug: OverflowError: cannot convert float infinity to integer
        log.info(
            f"{prefix}\n"
            + "\n".join(
                [
    ...
    def eval(self) -> Dict[str, Any]:
        ...
        for evaluator in self.evaluators:
            ...
            for eval_step, eval_batch in enumerate(eval_batches):
                if evaluator.type == "generation":
                    self.eval_generation_step(eval_batch, evaluator)
                else:
    ...
    def fit(self):
        ...
        if self.cfg.load_path is not None and self.global_step > 0 and self.cfg.eval_on_load:
            ...
            if get_global_rank() == 0 and self.should_log_to_swanlab():
        ...
        with torch_profiler as p:
            for epoch in range(self.epoch or 0, self.max_epochs):
                for batch in self.train_loader:
                    ...
                    if (
                        get_global_rank() == 0
                        and self.should_log_to_swanlab()

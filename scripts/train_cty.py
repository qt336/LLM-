...
from olmo.model_cty import OLMo
...
def main(cfg: TrainConfig) -> None:
    ...
    # Maybe start W&B run.
    # assert cfg.swanlab is not None, f"cfg.swanlab needs to be set (even if it's just `swanlab: {{}}`) for swanlab logging to work properly, but got {cfg.swanlab}"
    ...
    with Trainer(
        ...
        if cfg.load_path is not None:
            ...
            log.info("Checkpoint successfully loaded")
            # print(f"inv_freq_after_loading: {trainer.model.transformer.blocks[0].rotary_emb.inv_freq}")
            # exit() # debug
